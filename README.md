...
Same layout and visual language as [M2_LOG](https://github.com/M2Station/M2_LOG),
adapted for prompt engineering. Fill in the project fields on the left, add any
number of **function tabs** on the right (System / User / Context / Examples /
custom) and type your prompt, then hit **Export** — the app writes a timestamped
folder with `info.json`, a combined `prompt.md`, and one file per section. A
separate **PROMPT Library** tab lets you browse, track and re-open every prompt
you have exported.
...
### PROMPT Library (track & manage)
- Lists every exported prompt folder (newest first) with its timestamp.
- Type-to-filter the list.
- Click to preview `prompt.md`, copy it, or open its folder.
...
- Draggable splitters; all fields / sections persist via `localStorage`.
...
`info.json`、合併後的 `prompt.md`，以及每個分頁各一個檔案。另有 **PROMPT 管理**
分頁可瀏覽、追蹤並重新開啟你輸出過的每一個 Prompt。
...
### PROMPT 管理（追蹤與管理）
- 列出每一個輸出過的 Prompt 資料夾（最新在上）與時間戳。
- 輸入即時過濾清單。
- 點選即可預覽 `prompt.md`、複製內容或開啟資料夾。
